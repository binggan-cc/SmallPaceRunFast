...
│   ├── adapters/      # 项目适配器（JSON）
│   ├── models.py      # 核心数据模型
│   └── cli.py         # CLI 入口（19 条命令）
├── tests/             # 测试（1301 passed, 1 skipped）
...
Phase 11D Step 2 完成 — Scope Gate（21 MCP 工具，1301 tests）
...
### Phase 11D — Collaboration Handoff v0（Step 2 完成，Step 3 待实现）
- Step 3：handoff code（下一步默认从这里开始）
