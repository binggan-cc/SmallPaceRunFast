...
Step 2  dev.guard（R0）                                                ✅ 完成
        - smartdev/core/guard_dev.py（规则引擎）
        - smartdev/skills/dev_guard/skill.py + skill.yaml
        - 7 条 AI 编程硬规则检查
        - tests/test_guard_dev.py（88 tests）

Step 3  dependency.guard（R0）                                         ✅ 完成
...
Step 6  GuardRunner 组合 + CLI 入口（R0）                                  ✅ 完成
        - smartdev/core/guard_runner.py（一键全跑 + 聚合报告）
        - CLI: smartdev guard run [--select change.budget,dev.guard]
        - tests/test_guard_runner.py（21 tests）

Step 7  MCP 暴露只读 Guard 工具（R0）                                      ✅ 完成
        ...
        - tests/test_mcp_guard_tools.py（26 tests）
```
